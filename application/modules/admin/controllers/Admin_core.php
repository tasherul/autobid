...
class Admin_core extends CI_Controller
{
    ...
    {
        $nid_or_passport = $_FILES['nid_or_passport']['name'];
        echo $nid_or_passport;
        die;
        if (!empty($nid_or_passport)) {
            $config = [
                'upload_path' => './uploads/images/',
                'allowed_types' => 'jpg|png|jpeg|webp|gif|svg',
                'encrypt_name' => true,
            ];

            $this->load->library('upload', $config);

            if ($this->upload->do_upload('nid_or_passport')) {
                $file_name = $this->upload->data('file_name');
                $userdata['nid_or_passport'] = $file_name;
                $this->db->where('id', $this->input->post('user_id'))->update('users', ['nid_or_passport' => $nid_or_passport]);
                echo json_encode(site_url('admin/editprofile'));
            } else {
                echo json_encode(site_url('admin/editprofile/' . '?file=' . $this->upload->error()));
                die;
            }
        } else {
            echo json_encode(site_url('admin/editprofile/' . '?file=Upload NID or Passport!'));
            die;
        }
    }
