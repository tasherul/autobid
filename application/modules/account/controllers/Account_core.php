...
class Account_core extends CI_Controller
{
    ...
    public function signup($file = '')
    {
        ...
        $data['content']     = load_view('register_view', ['file' => $file], TRUE);
        $data['alias']        = 'signup';
        load_template($data, $this->active_theme);
    }
    ...
    function register()
    {
        ...
        if ($this->form_validation->run() == FALSE && $nid_or_passport != false) {
            $this->signup();
        } else {

            $this->load->library('bcrypt');

            $userdata['user_type']    = $this->input->post('user_type'); //2 = users
            ...
            if (!empty($nid_or_passport)) {
                ...
                if ($this->upload->do_upload('nid_or_passport')) {
                    ...
                    redirect($_SERVER['HTTP_REFERER'] . '?file=' . $this->upload->error());
                    die;
                }
            } else {
                $this->signup('Upload NID or Passport!');
                die;
            }
            $userdata['status']        = 1;
