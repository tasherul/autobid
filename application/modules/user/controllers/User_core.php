    ...
    public function create_ad()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (constant("ENVIRONMENT") == 'demo') {
                ...
                // Auction Section 
                if ($this->input->post('is_auction')) {
                    $auction_data = [
                        'car_id' => $post_id,
                        'asking_bid' => $this->input->post('asking_bid'),
                        'start_time' => $this->input->post('start_time'),
                        'end_time' => $this->input->post('end_time'),
                    ];
                    if ($this->session->userdata('user_type') == 1) {
                        $auction_data['auction_status'] = 1;
                    }
                    $this->db->insert('auction', $auction_data);
                    $this->db->where('id', $post_id)->update('posts', ['is_auction' => 1]);
                } else {
                    $this->db->where('id', $post_id)->update('posts', ['is_auction' => 0]);
                }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (constant("ENVIRONMENT") == 'demo') {
                ...
                if ($this->input->post('is_auction')) {
                    $auction_data = [
                        'car_id' => $post_id,
                        'min_bid' => $this->input->post('min_bid'),
                        'start_time' => $this->input->post('start_time'),
                        'end_time' => $this->input->post('end_time'),
                    ];
                    if ($this->session->userdata('user_type') == 1) {
                    ...
                    $exist_auction = $this->db->where('car_id', $post_id)->get('auction')->row();
                    if (!empty($exist_auction)) {
                        $this->db->where('car_id', $post_id)->update('auction', $auction_data);
                    } else {
                        $this->db->insert('auction', $auction_data);
                    }
                    $this->db->where('id', $post_id)->update('posts', ['is_auction' => 1]);
