                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade in active" id="p-nav-1">
                            ...
                            <div class="single-property">
                                ...
                                <div class="clearfix"></div>
                                <?php $auction = $this->db->where('car_id', $post->id)->get('auction')->row();
                                if (!empty($auction) && $auction->start_time <= date('Y-m-d h:i:s') && $auction->end_time >= date('Y-m-d h:i:s')) {
                                    $highest_bid = $this->db->order_by('amount', 'DESC')->get_where('bid', ['car_id' => $auction->car_id])->row();
                                ?>
                                    <div class="timer-div">
                                        <div style="display: inline-flex;align-self: center;font-size: 20px; font-weight: 500;">Auction Will End:</div>
                                        <ul id="timer" style="display: flex;">
                                            <li class="circle days"></li> :
                                    ...
                                    <p style="text-align: center;margin: 20px 0px;">Starting Bid: <span style="font-size: 20px; font-weight: 500;">$<?= number_format($auction->min_bid, 2) ?></span></p>
                                    <?php
                                    if (!empty($highest_bid)) {
                                        $user = $this->db->where('id', $highest_bid->user_id)->get('users')->row();
                                    ?>
                                        <marquee behavior="smooth" direction="up" style="height: 25px;" scrollamount="2">
                                            <p style="text-align: center;">Highest Bid: <span style="font-size: 20px; font-weight: 500;"><?= !empty($user) ? $user->first_name : '' ?> <span class="text-success">$<?= number_format($highest_bid->amount, 2) ?></span></span></p>
                                        </marquee>
                                    <?php }

                                    if (!empty($this->session->userdata('user_id'))) { ?>
                                        <div class="text-center" style="margin: 20px 0px;">
                                            <button type="button" id="startBid" class="btn btn-danger"><i class="fa fa-gavel" style="transform: rotate(90deg);"></i> Start Bidding</button>
                                        </div>

                                    <?php } else { ?>
                                        <div class="text-center" style="margin: 20px 0px;">
                                            <button type="button" class="btn btn-danger signin"><i class="fa fa-gavel" style="transform: rotate(90deg);"></i> Let's Bid</button>
                                        </div>
                                <?php }
                                }
                                ?>
...
<div class="modal fade" id="bidModal" data-keyboard="false" tabindex="-1" aria-labelledby="bidModal" aria-hidden="true">
    <div class="modal-dialog">
        <form action="<?= base_url('admin/auction/add_bid') ?>" method="POST">
            <input type="hidden" name="car_id" id="car_id" value="<?= $post->id ?>">
            <input type="hidden" name="auction_id" id="auction_id" value="<?= isset($auction) && !empty($auction->auction_id) ? $auction->auction_id : 0 ?>">
            <div class="modal-content">
                <div class="modal-header  login-modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                    <h4 class="modal-title" id="staticBackdropLabel">Bid For Product (Min Bid Amount: <span id="minAmount"><?= !empty($auction) ? $auction->min_bid : 0 ?></span> )</h4>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="">Place Bid Price*</label>
                        <input type="number" name="amount" id="amount" class="form-control" placeholder="Enter Amount" min="<?= !empty($auction) ? $auction->min_bid : 0 ?>" required>
                        <p style="color: red; font-size: 13px; font-weight: 500; margin: 10px 0px;">Payable: BDT 499 + 2 % Service Charge</p>
                    </div>
                    <div class="form-group">
                        <input type="checkbox" id="agreed" required> <label for="agreed" style="color: #7f232c; cursor: pointer;">I Agree Bid terms and conditions</label>
                    </div>
                </div>
                <div class="modal-footer">
...
<script>
    ...
    var countDownDate = new Date("<?= !empty($auction) && !empty($auction->end_time) ? date('M d, Y h:i:s', strtotime($auction->end_time)) : '' ?> <?= $timezone == 'UTC' ? 'UTC' : '' ?>").getTime();
